...
available [here](https://isaac-sim.github.io/IsaacLab/main/source/overview/environments.html#locomotion).
...
1. Train:  
   ```python legged_gym/scripts/train.py --task=anymal_c_flat```
    ...
    - **Important**: To improve performance, once the training starts press `v` to stop the rendering. You can then
      enable it later to check the progress.
    - The trained policy is saved in
      `issacgym_anymal/logs/<experiment_name>/<date_time>_<run_name>/model_<iteration>.pt`. Where `<experiment_name>`
      and `<run_name>` are defined in the train config.
...
`cleanup_models.py` 脚本用于清理日志文件夹中的旧 epoch 模型文件，只保留每个训练会话中最新的模型。

## 主要特性

- **智能识别**：自动识别 `logs/` 目录下的所有任务和训练会话
- **灵活选择**：支持单选、多选或全选任务进行清理
- **安全预览**：默认为预览模式，显示将要删除的文件列表
- **空间统计**：计算并显示将释放的磁盘空间
- **安全确认**：执行删除前需要用户明确确认
...
然后按提示输入任务编号（可以输入多个，用逗号分隔）

#### 直接指定单个任务

```bash
python cleanup_models.py --tasks rough_Go2
```

#### 直接指定多个任务

```bash
python cleanup_models.py --tasks rough_Go2 rough_Go2_ATT rough_Go2_bio
```
...
### 3. 执行删除（添加 --execute 参数）

#### 删除单个任务

```bash
python cleanup_models.py --tasks rough_Go2 --execute
```

#### 删除多个任务

```bash
python cleanup_models.py --tasks rough_Go2 rough_Go2_ATT --execute
```

#### 删除所有任务

```bash
python cleanup_models.py --all --execute
```

### 4. 自定义日志目录

```bash
python cleanup_models.py --log-dir /path/to/your/logs --all
```
